...
不需要真的发版，用 `scripts/dev-update-server.mjs` 起一个伪 Release 服务：
...
`ANDDRIVE_UPDATE_FORCE=1` 让更新器在非打包（dev）环境也运行。dev 下点「更新重启」不替换应用，而是直接预览「更新内容」。真实替换与重启只在打包环境发生（见 README「本地测试更新」第 3 步）。
