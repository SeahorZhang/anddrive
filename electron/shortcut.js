...
//
// 文件关联：
// - 打包版：electron-builder.json 的 fileAssociations 已把后缀写进 Info.plist。
...
function devLauncherMarker(exePath, projectPath, devServerUrl) {
  return JSON.stringify({ exePath, projectPath, devServerUrl, extension: MIRROR_FILE_EXTENSION });
}
...
async function removeDevLauncher() {
  const baseDir = path.join(app.getPath("appData"), DEV_USER_DATA_DIRNAME, DEV_LAUNCHER_DIRNAME);
  const bundlePath = path.join(baseDir, DEV_LAUNCHER_BUNDLE);
