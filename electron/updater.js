...
/**
 * Release endpoint. `ANDDRIVE_UPDATE_API` overrides it with a release-shaped
...
const updateLogPath = () => path.join(os.homedir(), 'Library', 'Logs', 'AndDrive', 'update.log')

async function logUpdateFailure(error) {
  ...
  try {
    ...
    console.warn('Failed to write update log')
  }
}
...
async function checkForUpdates() {
  ...
  try {
    ...
    if (!asset?.browser_download_url) {
      await logUpdateFailure(`release ${version} has no macOS update asset`)
      setStatus({ state: 'not-available', version, message: '' })
      return
    }
